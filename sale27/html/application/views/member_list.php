...
<script>
    function find_text()
	{
		if (!form1.text1.value)					// 값이 없는 경우, 전체 자료
			form1.action="/~sale27/member/lists";
		else                                    // 값이 있는 경우, text1 값 전달
			form1.action="/~sale27/member/lists/text1/" + form1.text1.value;
		form1.submit();
	}
</script>
...
?>
                <div class="col-9" align="right">           
                     <a href="/~sale27/member/add<?=$tmp; ?>" class="btn btn-sm mycolor1">추가</a>
                </div>
            </div>
        </form>
...
?>
			<tr>
				...
				<td><?=$row->uid27; ?></td>
				<td><?=$row->pwd27; ?></td>
				<td><a href="/~sale27/member/view/no/<?=$no; ?><?=$tmp; ?>"><?=$row->name27; ?></a></td>
				<td><?=$tel; ?></td>
				<td><?=$rank; ?></td>
			</tr>
<?
