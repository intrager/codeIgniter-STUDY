<?
    ...
    {
        public function getlist($text1,$start,$limit,$uid,$rank)
        {   
			if($uid && $rank==0) {
				$sql="select * from member1 where uid27='$uid'";    // 전체 자료
			} else if ($uid && $rank==1) {
				if (!$text1)
		...
		public function rowcount($text1, $uid, $rank)
		{
			if($uid && $rank==0) {
				$sql="select * from member1 where uid27='$uid'";
			} else if ($uid && $rank==1) {
				if(!$text1)
