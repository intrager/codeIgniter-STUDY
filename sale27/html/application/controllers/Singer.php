<?
    class Singer extends CI_Controller {       // singer클래스 선언
        function __construct()                           // 클래스생성할 때 초기설정
        {
            parent::__construct();
            $this->load->database();                     // 데이터베이스 연결
            $this->load->model("singer_m");				// 모델 singer_m 연결
        ...
        public function lists()
        {
		    ...
		    $text1 = array_key_exists("text1",$uri_array) ? urldecode($uri_array["text1"]) : "" ;
		 	...
		 	if ($text1=="") 
				$base_url = "/singer/lists/page";				  // $page_segment = 4;
			else
				$base_url = "/singer/lists/text1/$text1/page";    // $page_segment = 6;
			$page_segment = substr_count( substr($base_url,0,strpos($base_url,"page")) , "/" )+1;
			$base_url = "/~sale27" . $base_url;

			$config["per_page"]	 = 5;                              // 페이지당 표시할 line 수
			$config["total_rows"] = $this->singer_m->rowcount($text1);  // 전체 레코드개수 구하기
            ...
            $data["list"]=$this->singer_m->getlist($text1,$start,$limit);   // 해당페이지 자료읽기

            $this->load->view("main_header1");                    // 상단출력(메뉴)
            $this->load->view("singer_list",$data);           // singer_list에 자료전달
		...
		public function view()
		{
			...
			$data["text1"]=$text1;                      // text1 값 전달을 위한 처리
			$data["page"]=$page;
			$data["row"]=$this->singer_m->getrow($no);
			$data["list"]=$this->singer_m->getrecord_list($no);

			$this->load->view("main_header1");                    // 상단출력(메뉴)
            $this->load->view("singer_view",$data);				// singer_view에 자료전달
		...
		public function del()
		{
			...
			$result = $this->singer_m->deleterow($no);
		...
		public function add()
		{
			...
			{ 
				...
				$this->load->view("singer_add");    // 입력화면 표시
				$this->load->view("main_footer1");
			}
			else              // 입력화면의 저장버튼 클릭한 경우
			{
				$data = array(
					...
					"tmi27" => $this->input->post("tmi",TRUE)
				);
				$picname = $this->call_upload();		// 업로드 시작
				if($picname) $data["pic27"] = $picname;	// 파일이름 저장

				$result = $this->singer_m->insertrow($data); 

				redirect("/~sale27/singer/lists" . $text1 . $page);    //   목록화면으로 이동.
			}
		}
		...
		public function edit()
		{
			...
			if ( $this->form_validation->run()==FALSE )     // 수정버튼 클릭한 경우
			{
				$this->load->view("main_header1");
				$data["row"] = $this->singer_m->getrow($no);
				$this->load->view("singer_edit",$data);
				$this->load->view("main_footer1");
			}
			else           // 저장버튼 클릭한 경우
			{  
				...
				$result = $this->singer_m->updaterow($data,$no);
				redirect("/~sale27/singer/lists" . $text1 . $page);
			}
		}

		public function call_upload()
		{
			$config['upload_path'] = './singer_img';		// 저장할 경로
			...
			else 
			{
				...
				$config['source_image'] = './singer_img/' . $picname;	// 원본 사진 이름
				$config['thumb_marker'] = "";
				$config['new_image'] = './singer_img/thumb';	// thumb 저장 폴더
		...
		public function zoom()
		{
            ...
            $this->load->view("singer_zoom",$data);					// singer_zoom에 자료전달
            $this->load->view("main_footer1");                      // 하단 출력 
		}
	}
?>
