<?
    class Member1 extends CI_Controller {       // Member클래스 선언
        ...
        {
			if($this->session->userdata("rank")!=1) redirect("/main1");
        ...
        public function lists()
        {
			...
			$config["total_rows"] = $this->member_m1->rowcount($text1);  // 전체 레코드개수 구하기
            ...
            $data["list"]=$this->member_m1->getlist($text1,$start,$limit);   // 해당페이지 자료읽기

            $this->load->view("main_header1");                    // 상단출력(메뉴)
            $this->load->view("member_list1",$data);           // member_list에 자료전달
		...
		public function view()
		{
			...
			$this->load->view("main_header1");                    // 상단출력(메뉴)
            $this->load->view("member_view1",$data);           // member_list에 자료전달
		...
		{
			$uri_array=$this->uri->uri_to_assoc(3);
			$no=array_key_exists("no",$uri_array) ? $uri_array["no"] : "" ;		// URI: /~sale27/member/del/no/1
