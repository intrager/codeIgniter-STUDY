<?
    class Jangbuo extends CI_Controller {       // jangbuo클래스 선언
		...
		{
			$uri_array=$this->uri->uri_to_assoc(3);
			$no=array_key_exists("no",$uri_array) ? $uri_array["no"] : "" ;		// URI: /~sale27/jangbuo/del/no/1
			$text1=array_key_exists("text1",$uri_array) ? "/text1/" . urldecode($uri_array["text1"]) : "";
			$page = array_key_exists("page",$uri_array) ? "/page/" . urldecode($uri_array["page"]) : 0 ;
			
			$this->jangbuo_m->deleterow($no);

			redirect("/~sale27/jangbuo/lists". $text1 . $page);             // 목록화면으로 돌아가기
		}
		...
		public function add()
		{
			...
			else              // 입력화면의 저장버튼 클릭한 경우
			{
				$data = array(
					...
					"numi27" => 0,
					"numo27" => $this->input->post("numo",TRUE),
				...
				redirect("/~sale27/jangbuo/lists" . $text1 . $page);    //   목록화면으로 이동.
			}
		}
		...
		public function edit()
		{
			...
			{  
				...
				redirect("/~sale27/jangbuo/lists" . $text1 . $page);
			}
		}
	}
?>
